...
class HostedGameAgent implements IHostedGameAgent {
   ...
   * @returns
   */
  async listAvailableDefaultTwitterFunctions(): Promise<
  ...
  run(args: any): void {
    this.customFunctions.forEach((func) => {
      const hostedGame = new ExecutableHostedGameFunction(
        func.name,
        func.description,
        func.args,
        func.config,
